function checkCashRegister(price, cash, cid) {
  ...
  //Notre cash dispo en caisse
  let cashDispo = {};
  cid.map(valor => (cashDispo[valor[0]] = valor[1]));
  ...
  //function
  function rendre(monnaie, cashDispo) {
    if (monnaie > 0) {
      for (let valor in valors) {
        const val = valors[valor];
        while (monnaie > val){
          monnaie = monnaie - val;
        }
      console.log("il faut rendre:", monnaie);
    }
  }
    return change;
  }

  rendre(monnaie, cashDispo, valors, change);
